#!/usr/bin/env python3
"""Rank annual cumulative media objects by Asia-28 downloader weight share.
...
def reduce_sample(job):
    ...
    regional = dict(world)
    hosting = {'world': dict(world), 'asia28': dict(world)}
    for feature in geo['features']:
        ...
        for role in world:
            ...
            if code in asia:
                regional[role] += count
                hosting['asia28'][role] += hosted
    for role in world:
        assert sum(value[role] for value in countries.values()) == world[role]
        assert sum(value[role] for code, value in countries.items() if code in asia) == regional[role]
    ...
    return {'key': key, 'name': meta['collection_name'], 'collection_id': meta['collection_id'],
            ...
            'features': len(geo['features']), 'world': world, 'asia28': regional,
...
def share(record, role='downloaders'):
    return Fraction(record['asia28'][role], record['world'][role])
...
def render(data, output, ledger):
    ...
    lines = ['---', 'layout: default', f'title: "Top {count} media objects by Asia-28 share"',
             ...
             '[AAPI-Led results](../index.html)', '', f'# Top {count} media objects by Asia-28 share', '',
             ...
             'downloader weight located in **Asia-28**. Each row uses its full available sample window; '
             ...
             f'<div class="analysis-table-scroll" role="region" aria-label="Top {count} media objects by Asia-28 share" tabindex="0" markdown="1">', '',
             '| Rank | Media object | Sample window | Days | Asia-28 % | Asia-28 downloader weight | Worldwide downloader weight |',
             '| ---: | --- | --- | ---: | ---: | ---: | ---: |']
    for rank, row in enumerate(top, 1):
        label = row['name'] + (' · ' + str(row['collection_id']) if row['collection_id'] else '')
        label = label.replace('|', '\\|')
        lines.append(f'| {rank} | [{label}]({row["audit_url"]})<br>`{row["key"]}` | '
                     f'{row["start"]} to {row["end"]} | {row["sample_days"]:,} | '
                     f'**{float(share(row)) * 100:.2f}%** | {row["asia28"]["downloaders"]:,} | {row["world"]["downloaders"]:,} |')
    lines += ['', '</div>', '', '## Definition and method', '',
              '`Asia-28 % = 100 × Asia-28 cumulative downloader weight / worldwide cumulative downloader weight`.', '',
              '- Country membership follows the project’s [current Asia-28 definition]('
              + data['definition_source']['url'] + '). The 28 ISO-3 codes are: '
              + ', '.join('`' + code + '`' for code in data['asia28']) + '.',
    ...
    script = '../resources/rank-asia-28.py'
    lines += ['', '</div>', '', '## Evidence and reproduction', '',
              ...
              'python3 resources/rank-asia-28.py \\',
...
def main():
    ...
    asia = sorted(row['code'] for row in definition['country_sets']['asia-28'])
    assert len(asia) == len(set(asia)) == 28 and 'TWN' in asia
    jobs, versions = [], {}
    for year in range(2017, 2027):
        ...
        jobs.extend((repo, version, key, set(asia)) for key in sorted(keys))
    samples = []
    ...
    for key, rows in sorted(by_key.items()):
        ...
        if len(rows) > 1:
            slim = lambda r: {field: r[field] for field in ['repository_year', 'start', 'end', 'world', 'asia28']}
    ...
    data = {'schema_version': 1, 'analysis_date': date.today().isoformat(), 'asia28': asia, 'top_count': args.top,
            'definition_source': references[0], 'definition_id': definition['definition_id'],
            'repositories': versions, 'metric': 'Cumulative Asia-28 downloader weight / worldwide geographic downloader weight',
            ...
            'ranking': [{'rank': i, 'key': row['key'], 'repository_year': row['repository_year'],
                         'asia28_weight': row['asia28']['downloaders'], 'world_weight': row['world']['downloaders'],
                         'asia28_percent': float(share(row)) * 100} for i, row in enumerate(ranked, 1)]}
    ...
    for row in data['ranking'][:args.top]:
        print(row['rank'], row['key'], row['repository_year'], f'{row["asia28_percent"]:.4f}%', flush=True)
